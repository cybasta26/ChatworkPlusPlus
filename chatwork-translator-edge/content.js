(() => {
  ...
  function translateShared(text) {
    ...
    const promise = Promise.resolve().then(() => EdgeTranslation.translate(text));
  ...
  function render(element, state, text, error = false, failures = []) {
    if (!state.box) {
    ...
    label.textContent = CW_I18N.t("ui_000", [AutoSettings.languageName(settings.targetLanguage)]);
    ...
    if (failures.length) {
      ...
      notice.className = 'cw-zh-warning'; notice.setAttribute('role','status');
      notice.textContent = CW_I18N.t('translation_partial',[failures.length]);
      notice.title = failures.map(item => `${item.line}: ${item.name}: ${item.message}`).join('\n');
      state.box.append(notice);
    }
    if (error) {
      ...
      initialize.textContent = CW_I18N.t("ui_002");
      initialize.addEventListener('click', async () => {
        initialize.disabled = true;
        try {
          await EdgeTranslation.initialize(progress => {
            initialize.textContent = CW_I18N.t("ui_003", [Math.round(progress * 100)]);
          });
          for (const item of states.values()) if (item.status === 'error') item.status = 'waiting';
          pump();
        } catch (error) {
    ...
    for (const element of new Set([...exportVisible,...visible])) {
      ...
      translateShared(state.requestText)
        .then(result => {
          ...
          state.status = result.incomplete ? 'error' : 'done';
          render(element, state, state.restoreNames(result.text), !!result.incomplete, result.failures || []);
        })
        .catch(error => {
          if (!enabled || version !== state.version || !element.isConnected) return;
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    setEnabled(false);
    EdgeTranslation.configure(settings.sourceLanguage, settings.targetLanguage);
