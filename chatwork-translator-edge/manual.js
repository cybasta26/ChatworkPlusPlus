(() => {
  ...
  button.addEventListener('click', async () => {
    ...
    let session;
    try {
      if (typeof globalThis.Translator === 'undefined') throw new Error(CW_I18N.t("ui_025"));
      status.textContent = CW_I18N.t("ui_026");
      // Invoke create directly during the click: model download may require user activation.
      session = await Translator.create({sourceLanguage: source.value, targetLanguage: target.value, signal,
        monitor(m) { m.addEventListener('downloadprogress', event => {
          if (version !== generation) return;
          const progress = event.total ? event.loaded / event.total : event.loaded;
          status.textContent = CW_I18N.t("ui_003", [Math.round(progress * 100)]);
        }); }
      });
      if (version !== generation) return;
      status.textContent = CW_I18N.t("ui_027");
      const parts = [];
      for (const segment of segments) {
        ...
        parts.push(await ProtectedText.translatePart(segment, async source => {
          ...
          if (version !== generation || signal.aborted) throw new Error(CW_I18N.t("ui_028"));
          if (typeof translated !== 'string' || !translated.trim()) throw new Error(CW_I18N.t("ui_029"));
          return translated;
        }));
      }
      output.value = parts.join('');
      copy.disabled = false;
      status.textContent = CW_I18N.t("ui_030");
    } catch (error) {
      if (version === generation) status.textContent = CW_I18N.t("ui_031", [error.message || CW_I18N.t("ui_032")]);
    } finally {
      session?.destroy();
