...
test('手动翻译开关、语言、独立运行和过期结果保护', async () => {
  ...
  w.Translator={create:async options=>{calls.push(options);return {translate:async text=>slow ? new Promise(resolve=>delayed=resolve) : '日本語:'+text,destroy(){}}}};
  ...
  try {
    ...
    get('enabled').checked=true;change('enabled');await settle();delete w.Translator;get('translate').click();await settle();
    assert.match(get('status').textContent,/未开放/);assert.equal(get('translate').disabled,false);
